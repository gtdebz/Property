...
<h2 class="text-center">Add a Property</h2>
	...
	for ($i=0; $i <$c; $i++){
	   ...
	   $ins=mysqli_query($db, $querymulti);   //result of query $querymulti over connection $db next display result in while function if u wish
	           if($ins){
				   ...
				   echo  "<h4 class='text-success'>Successfully added new property !!</h4>" ;
			   }else{
				   
				   echo  "<h4 class='text-danger'>Could not add new property please try again..</h4>" ;
			   }
